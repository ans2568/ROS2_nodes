...
def generate_launch_description():
    ...
    primary_imu_enable = EnvironmentVariable('JACKAL_IMU', default_value='true')
    primary_imu_model = EnvironmentVariable('JACKAL_IMU_MODEL', default_value='mi')
    primary_imu_port = EnvironmentVariable('JACKAL_IMU_PORT', default_value='/dev/ttyUSB0')
    primary_imu_mount = EnvironmentVariable('JACKAL_IMU_MOUNT', default_value='imu_link')

    if (primary_imu_enable.perform(lc)) == 'true':
        if (primary_imu_model.perform(lc) == 'mi'):
            node_mi_driver = Node(
                package="mi_ros2",
